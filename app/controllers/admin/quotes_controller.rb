class Admin::QuotesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @quote.save
        ...
        format.json { render :show, status: :created, location: [:admin, @quote] }
      else
        format.html { render :new }
        format.json { render json: [:admin, @quote.errors], status: :unprocessable_entity }
      end
    end
  end
  ...
  private
    ...
    def quote_params
      params.require(:quote).permit(:modelo_veiculo, :ano_veiculo, :placa_veiculo, :valor_fipe, :name, :email, :rg, :cpf, :telefone, :status_atendimento)
    end
end
